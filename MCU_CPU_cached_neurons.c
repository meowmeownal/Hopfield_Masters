...
static float Y4[T+1] = {0.7f};
	...
	static float Y0_sum[T+1];

	__attribute__((section(".ccmram")))
	static float Y1_sum[T+1];

	__attribute__((section(".ccmram")))
	static float Y2_sum[T+1];

	__attribute__((section(".ccmram")))
	static float Y3_sum[T+1];

//	__attribute__((section(".ccmram")))
	static float Y4_sum[T+1];
#endif
...
//------------precomputing BG function-------------

//float BG_cache[T+1];
//
//void init_BG_cache(float ni)
//{
...
//        } else {
//            float lg1 = lgammaf(k);
//            float lg2 = lgammaf(ni);
//            float lg3 = lgammaf(k + ni);
//            float beta = expf(lg1 + lg2 - lg3);
//            BG_cache[k] = 1.0 / (k * beta);
//        }
//    }
//}
...
//     return 1.0 - a1 * fast_tanh(y);
// }

// float G2(float y, float a2, float a3) {
//     return a2 - a3 * fast_sin(y);
...
//--------------DAC DATA CONVERTION----------------------------
    ...
    const float y_max = 2.5f;

    uint16_t dac_value(float y)
    {
...
//        }
//
//        if(y < y_min) y_min = y - y*0.2;
//        if(y > y_max) y_max = y + y*0.2;
//
//        if(fabsf(y_max - y_min) < 0.00001f)
//            y_max = y_min + 0.001f;

    	//y = fmaxf(y_min, fminf(y_max, y));
        float normalized = (y + fabsf(y_min)) / (y_max - y_min);
...
void simulate_realtime(float a2, float a3, float a1,
                      const float weights[N][N], float f1, float f3,
                      float ni,
                      float *restrict y0, float *restrict y1,
                      float *restrict y2, float *restrict y3,
                      float *restrict y4,
					  float *restrict y0_sum, float *restrict y1_sum,
                      float *restrict y2_sum, float *restrict y3_sum,
                      float *restrict y4_sum)
#else
void simulate_realtime(float a2, float a3, float a1,
                      const float weights[N][N], float f1, float f3,
                      float ni,
                      float *restrict y0, float *restrict y1,
                      float *restrict y2, float *restrict y3,
                      float *restrict y4, float *restrict y0_sum, float *restrict y1_sum,
                      float *restrict y2_sum, float *restrict y3_sum,
                      float *restrict y4_sum,
                      FILE *fp)
#endif
...
//    static float bgs[T+1];
//
//	bgs[0]=1.0f;
//
//	for (int k = 1; k <= T; k++)
//	{
//		bgs[k] = bgs[k - 1] * ( (k - 1 + ni) / k );
//	}


    float G1r0 = 1.0f - a1 * fast_tanh(y2[0]);
    float G2r0 = a2 - a3 * fast_sin(y4[0]);

	y0_sum[0] = (-y0[0] + weights[0][1] * fast_tanh(y1[0]) + weights[0][2] * fast_sin(y2[0]) + weights[0][3] * fast_sin(y3[0]));
	y1_sum[0] = (-y1[0] + weights[1][0] * fast_sin(y0[0]) + weights[1][2] * fast_sin(y2[0]) + weights[1][4] * fast_sin(y4[0]) + f1) ;
	y2_sum[0] = (-y2[0] + weights[2][0] * fast_tanh(y0[0]) + weights[2][1] * fast_tanh(y1[0]) + weights[2][2] * fast_sin(y2[0])) ;
	y3_sum[0] = (-y3[0] + weights[3][0] * fast_tanh(y0[0]) + G2r0 * fast_tanh(y3[0]) + f3);
	y4_sum[0] = (-y4[0] + weights[4][1] * fast_tanh(y1[0]) + G1r0 * fast_tanh(y4[0]));
    ...
    for (uint16_t om = 1; om < T+1; om++)
    {
        ...
        for (uint16_t r = 1; r < om + 1; r++)
        {
            ...
            // y3[om] += (-y3[r-1] + weights[3][0] * fast_tanh(y0[r-1]) + G2r * fast_tanh(y3[r-1]) + f3) * bg;
            // y4[om] += (-y4[r-1] + weights[4][1] * fast_tanh(y1[r-1]) + G1r * fast_tanh(y4[r-1])) * bg;
            y0[om] += y0_sum[r-1]*bg;
            y1[om] += y1_sum[r-1]*bg;
            y2[om] += y2_sum[r-1]*bg;
            y3[om] += y3_sum[r-1]*bg;
        ...
        float G1r = 1.0f - a1 * fast_tanh(y2[om]);
        float G2r = a2 - a3 * fast_sin(y4[om]);

        y0_sum[0] = (-y0[om] + weights[0][1] * fast_tanh(y1[om]) + weights[0][2] * fast_sin(y2[om]) + weights[0][3] * fast_sin(y3[om]));
        y1_sum[0] = (-y1[om] + weights[1][0] * fast_sin(y0[om]) + weights[1][2] * fast_sin(y2[om]) + weights[1][4] * fast_sin(y4[om]) + f1) ;
        y2_sum[0] = (-y2[om] + weights[2][0] * fast_tanh(y0[om]) + weights[2][1] * fast_tanh(y1[om]) + weights[2][2] * fast_sin(y2[om])) ;
        y3_sum[0] = (-y3[om] + weights[3][0] * fast_tanh(y0[om]) + G2r * fast_tanh(y3[om]) + f3);
        y4_sum[0] = (-y4[om] + weights[4][1] * fast_tanh(y1[om]) + G1r * fast_tanh(y4[om]));

        #ifdef MCU

            uint32_t delay_step = 10; //by how many steps delayed
            ...
            uint16_t y0_data = dac_value(y0[om]);
            uint16_t y1_data = dac_value(y1[om]);
            uint16_t y2_data = dac_value(y2[om]);
            uint16_t y3_data = dac_value(y3[om]);
            uint16_t y4_data = dac_value(y4[om]);
            ...
            HAL_I2C_Master_Transmit(&hi2c1, DAC1_ADDR, data0, 2, HAL_MAX_DELAY); //data may need to be converted somehow ig
            if(om >= delay_step)
            {
                uint8_t data_delay[2] = {0};
                uint16_t y_delayed = dac_value(y0[om - delay_step]);

                two_bytes_data(data_delay,  y_delayed);
                HAL_I2C_Master_Transmit(&hi2c1, DAC2_ADDR, data_delay, 2, HAL_MAX_DELAY);

            }
            //HAL_I2C_Master_Transmit(&hi2c1, DAC2_ADDR, data1, 3, HAL_MAX_DELAY); //przedostatni argument to ilosc wysylanych bajtów, wysyla sie 2 wiec chyba trzeba mzienic
    ...
    #ifndef MCU
        //for(uint16_t i = T + 1 - 1000; i < T + 1; i++)
        for(uint16_t i = 0 ; i < T + 1; i++)  //if i = 0, will show initial vals
        {
            //uint16_t y0_data = dac_value(y0[i]);
...
int main()
{
    ...
    #ifndef MCU
        FILE *fp = fopen("float_precomp_338.csv", "w");
    #endif

     const float f1 = 0.0;
     const float f3 = 0.0;
     const float a1 = -2.2;
     const float a2 = -3.38; //changing that
     const float a3 = 1.2;
...
//    float y4[T+1] = {0.7};
    ...
    #ifdef MCU
        simulate_realtime(a2, a3, a1, weights, f1, f3, 0.7, Y0, Y1, Y2, Y3, Y4, Y0_sum, Y1_sum, Y2_sum, Y3_sum, Y4_sum);
    #else
        simulate_realtime(a2, a3, a1, weights, f1, f3, 0.7, Y0, Y1, Y2, Y3, Y4, Y0_sum, Y1_sum, Y2_sum, Y3_sum, Y4_sum, fp);
    #endif
